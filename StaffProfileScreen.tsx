...
export default function StaffProfileScreen({ navigation }: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const cleanPhone = phone.trim() || me?.phone || undefined;
  ...
  const roleLabel = me?.staffRole === 'sales_executive' ? 'Sales Executive' : me?.staffRole === 'driver' ? 'Driver' : 'Staff';
  const dealershipLine = [me?.dealershipName, me?.dealershipLocation].filter(Boolean).join(' • ');
      ...
      <SafeAreaView edges={['top']} style={styles.safe}>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryLabel}>Access level</Text>
            <Text style={styles.summaryValue}>{me?.accessLevel || 'limited_access'}</Text>
            <Text style={styles.summaryLabel}>Dealership</Text>
            <Text style={styles.summaryValue}>{dealershipLine || 'Unassigned'}</Text>
          </View>
          ...
          <TextInput style={styles.input} value={birthday} onChangeText={setBirthday} placeholder="YYYY-MM-DD" placeholderTextColor={colors.textLight} />
      ...
      <Modal visible={photoModalVisible} transparent animationType="fade" onRequestClose={() => setPhotoModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.photoModal}>
            ...
            <Text style={styles.photoModalText}>Choose a new picture for your staff profile.</Text>
...
const styles = StyleSheet.create({
  ...
  summaryCard: { backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.borderLight, padding: spacing.lg, marginBottom: spacing.lg },
  summaryLabel: { fontSize: 11, fontWeight: '900', color: colors.textLight, textTransform: 'uppercase', marginTop: spacing.md, marginBottom: 4 },
